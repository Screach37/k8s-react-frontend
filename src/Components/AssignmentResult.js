...
export default function AssignmemtDeclared() {
  ...
  // console.log("id",id);
    ...
    const [assign_pap, setassign_pap] = useState("NA");
    const [stu_res, setstu_res] = useState("");
    const [status, setstatus] = useState();
    const [tot_mark, settot_mark] = useState();
        ...
        fetch(`${API_URL}/view/completed/assignments/dec`, requestOptions)
          ...
          .then(result => {
            if(result.status == "1"){
              ...
              settot_mark(result.sending.assignment_det.tot_mark);
              ...
              setmarks(result.sending.assignmnet_res_det.marks);
              setstatus(result.sending.assignmnet_res_det.status);
            }
          })
  ...
  return (
    <div>
           ...
           <div className='row'>
             <div className='col-12' style={{ display:'flex', justifyContent:"space-between", alignItems:'center' }}>
              <div className='d-flex align-items-center' >
                 ...
                 <div className='up-assi-txt' style={{ fontSize:14, fontWeight:500, color:"#858585", fontFamily:'inter' }} >{sch_time?.split('T')[0]}</div>
              </div>
              <div className='d-flex align-items-center' >
                 <CiClock2 className='sub-assi-txt' style={{ fontSize:20, color:"#545454" }} />
                 <div className='up-assi-txt' style={{ fontSize:14, fontWeight:500, color:"#858585", fontFamily:'inter' }} >{sch_time?.split('T')[1].split('.')[0]} - {due_time?.split('T')[1].split('.')[0]}</div>
              </div>
             </div>
           </div>
      ...
      <div className='row px-3 mt-4'>
          <div className='col-5 text-start assi-declr-txt' style={{ fontSize:20, fontWeight:600, color:"#545454" }} >Result : <span className='assi-declr-txt' style={{ fontSize:20, fontWeight:600, color:"#0CBC8B" }}>{marks}</span>/{tot_mark}</div>
        <div className='col-5 ms-auto assi-declr-txt'>
            {
              status == "fail" ?(<div className='p-1' style={{ color:"red", fontSize:13, fontWeight:500, backgroundColor:"#FEECEA", borderRadius:5 }} >Result : {status}</div>):
              (<div className='p-1' style={{ color:"#0CBC8B", fontSize:13, fontWeight:500, backgroundColor:"#E1FFEF", borderRadius:5 }} >Result : {status}</div>)
            }
        </div>
      </div>
